...
class Simulation:
    def __init__(
        self, graph: NetworkGraph, nb_drones: int, start_hub: str, end_hub: str
    ):
        self.graph = graph
    ...
    def evaluate_best_path(self, current_hub: str, start_turn: int) -> list:
        ...
        for path_data in all_paths:
            ...
            if len(path) > 1:
                ...
                if conn_obj:
                    drones_na_conn = self.conn_occupancy.get(start_turn, {}).get(
                        conn_key, 0
                    )
    ...
    def simulate_path_arrival(self, path: list, start_turn: int) -> int:
        ...
        for i in range(1, len(path)):
            ...
            while True:
                ...
                drones_no_hub = self.hub_occupancy.get(arrival_time, {}).get(
                    next_hub_name, 0
                )
    ...
    def move_drone(self, drone):
        ...
        if hub_ok and conn_ok:
            ...
            drone["status"] = "in_transit"
            print(
                f"✈️ Drone {drone['id']} descolou de {current_hub_name} para {next_hub_name} (Previsão: Turno {arrival_time})"
            )
            ...
            print(
                f"⏱️ Drone {drone['id']} retido em {current_hub_name}. Adiando partida para o próximo turno ({motivo} para o Turno {arrival_time})."
            )
            drone["lock_until_turn"] = self.current_turn + 1
            drone["is_flying"] = False
            drone["status"] = "landed"
